...
05_simulation.py
─────────────────
Simuliert Dynamic Pricing vs. Statisches Pricing über 30 Tage.
Berechnet Umsatz, Marge und Gewinnprognose für beide Strategien.
"""
...
def run_simulation(X_test, y_test, best_model, n_products=300,
    ...
    """
    Simuliert 30 Tage Dynamic vs. Statisches Pricing.

    Parameters
    ----------
    X_test      : Test-Features
    y_test      : Test-Zielvariable (log-Skala)
    best_model  : trainiertes ML-Modell
    n_products  : Anzahl simulierter Produkte
    n_steps     : Zeitschritte (Tage)
    cost_rate   : Einkaufspreis als Anteil des Listenpreises (Standard: 60%)
    out_dir     : Ausgabeordner für Plots und CSV

    Returns
    -------
    results_df  : pd.DataFrame mit täglichen Umsatz- und Margen-Werten
    summary     : dict mit Gesamtkennzahlen
    """
    ...
    # Statischer Preis = 85% des Listenpreises (typischer fixer Rabatt)
    static_price = np.expm1(X_sim["log_act_price"].values) * 0.85
    # Einkaufspreis = 60% des Listenpreises
    cost = static_price * cost_rate  # Einkaufspreis = 60% des Verkaufspreises
